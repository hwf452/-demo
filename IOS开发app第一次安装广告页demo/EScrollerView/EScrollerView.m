...
-(id)initWithFrameRect:(CGRect)rect ImageArray:(NSArray *)imgArr TitleArray:(NSArray *)titArr
{
	...
	if ((self=[super initWithFrame:rect])) {
        ...
        for (int i=0; i<pageCount; i++) {
            ...
            if (i==pageCount-1) {
                UIButton *btn=[[UIButton alloc] initWithFrame:CGRectMake(viewSize.size.width/2-50, viewSize.size.height-220, 100, 30)];
                
                [btn setBackgroundImage:[UIImage imageNamed:@"kstybutton.png"] forState:UIControlStateNormal];
...
-(void)btnClick:(UIButton *)btn{
    ...
    //NSLog(@"sdksksfdsf");
    if ([delegate respondsToSelector:@selector(btnClickdel:)]) {
        [delegate btnClickdel:btnFast];
    }
